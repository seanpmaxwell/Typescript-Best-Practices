...
### Classes
- Use these for managing objects that are generally instantiated many times throughout your application, and have both dynamic and static (that includes functions) properties. For example, `Promise` is a class cause we make many promises when doing IO calls, and it has dynamic properties like `state` and `result`. `jet-logger` (the logging tool that I use in NodeJS) is just a function that returns a basic object because a logger, while can theoretically be instantiated many times, is generally instantied once and shared throughout the entire application.
